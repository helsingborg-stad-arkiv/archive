...
async function init() {
  ...
  domainSelect.innerHTML = '<option value="">-- Select site --</option>' + domains.map((d) => `<option value="${d}">${d}</option>`).join("");
  ...
  __name(updateUrlDisplay, "updateUrlDisplay");
  ...
  function loadArchive() {
    ...
    const baseSrc2 = `./${domain}/${date}/${domain}/index.html`;
    preview.src = baseSrc2;
    localStorage.setItem("lastDomain", domain);
    localStorage.setItem("lastDate", date);
    updateUrlDisplay(baseSrc2, domain, date);
  }
  ...
  __name(preventExternalTargets, "preventExternalTargets");
